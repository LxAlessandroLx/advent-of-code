...
public:
    ...
    void addItem(const long item) { items.push(item); }
    bool hasItems() { return !items.empty(); }
    size_t getInspectedCount() { return inspectedCount; }
    ...
    void inspect()
    {
        ...
        currentItem = ((currentItem + addFactor) * multiplyFactor) % rationalizer;
    ...
    long addFactor, multiplyFactor, testFactor;
...
private:
    ...
    static void parseOperation(std::ifstream & file, Monkey & monkey)
    {
        ...
        {
            long factor = std::stoi(line.substr(1));
    ...
    static void parseTest(std::ifstream & file, Monkey & monkey)
    {
        ...
        if (rationalizer % monkey.testFactor != 0)
            rationalizer *= monkey.testFactor;
    }
